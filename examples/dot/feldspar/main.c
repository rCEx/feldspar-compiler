...
int main (int argc, char *argv[]) {
  ...
  struct array *a = NULL;
  struct array *b = NULL;
  a = initArray(a, sizeof(uint64_t), arrSize); 
  b = initArray(b, sizeof(uint64_t), arrSize); 
  ...
  res = initArray(res, sizeof(uint64_t), arrSize);
  ...
  {
    printf("%d ", at(uint64_t,a,i));
    printf("%d ", at(uint64_t,b,i));
  }
  printf("\n");
  ...
  outputMeasure("dotFeldspar.log",nanos, arrSize);

  printf("After:  ");
  for (int i=0; i<min(arrSize,10); i++)
  {
    printf("%d ", at(uint64_t,res,i));
  }
  printf("\nres:%d\n",at(uint64_t,res,0));
